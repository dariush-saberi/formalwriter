...
def processText(text):
    ...
    for x in enumerate(mywords):
        if x[1] not in acadz.stops and x[1] not in "(),;." and mytags[x[0]] != "NNP" and x[1].lower() not in acadz.stops and not any(char.isdigit() for char in x[1]):
            sent = mywords[:]
            sent[x[0]] = "[MASK]"
...
def formatting(text,words):
    for x in words:
        ...
        for w in keysList:
            ...
            select.append(pprt)
        text[keyindex] = "<li class='dropdown' style='display:inline-block'><span class='dropdown-toggle' data-toggle='dropdown' style='text-decoration:underline;color:{}'>{}</span><ul class='dropdown-menu' role='menu' aria-labelledby='menu1'>{}</ul></li>".format(mycolor,myword,"".join(select))
    return " ".join(text)
