...
def bert_mask300(sent):
    ...
    for index in indices:
        ...
        predictions = [
            bert_tokenizer.convert_ids_to_tokens(x.tolist()) for x in best_words
        ]
...
def xlnet_sent_score_OLD3(text):  # new code based on xlnet working code in my drive
  ...
  man is chased outside and beaten. <eod> """
    ...
    sum_lp = 0.0
    for max_word_id in range(
        (len(tokenize_input) - len(tokenize_text)), (len(tokenize_input))
    ):
        sent = tokenize_input[:]
        ...
        word_id = xlnet_tokenizer.convert_tokens_to_ids([tokenize_input[max_word_id]])[
            0
        ]
        # print(word_id)
