...
def bert_mask_predict(sent):
    ...
    for index in indices:
        ...
        predictions = [langmodel.bert_tokenizer.convert_ids_to_tokens(x.tolist()) for x in best_words]
...
class Expression:
    ...
    engPOSPR = {'my':'i','your':'you','his':'he','her':'she','its':'it','our':'we','their':'they'}
    ...
    def passiveConverter(self):
        ...
        elif self.verb and not self.active and self.time == "present" and not self.plural:
    ...
    def clauseMaker(self):
        ...
        if self.verb and self.time == "past":
            res = "{} {} {} {}".format(str(self.subject),str(self.verbPast),str(self.object),str(self.remain))
            self.result.append(res)
            print("1",res)
        elif self.verb and not self.plural and self.time == "present":    
            res = "{} {} {}".format(str(self.subject),str(self.verbThirdP),str(self.object),str(self.remain))
            self.result.append(res)
            print("2",res)
        elif self.verb and self.plural and self.time == "present":    
            res = "{} {} {}".format(str(self.subject),str(self.verbPresent),str(self.object),str(self.remain))
            self.result.append(res)
            print("3",res)
            
        elif self.adject and self.subCase == 1 and not self.plural and self.time == "past":
            res = "{} was {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("4",res)
        elif self.adject and self.subCase == 1 and self.plural and self.time == "past":
            res = "{} were {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("5",res)
            
        elif self.adject and self.subCase == 1 and not self.plural and self.time == "present":
            res = "{} is {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("6",res)
        elif self.adject and self.subCase == 1 and self.plural and self.time == "present":
            res = "{} are {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("7",res)
            ...
            self.result.append(res)
            print("8",res)
        elif self.adject and self.subCase == 2 and not self.plural and self.time == "past":
            res = "{} was {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("9",res)
        elif self.adject and self.subCase == 2 and self.plural and self.time == "present":
            res = "{} are {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("10",res)
        elif self.adject and self.subCase == 2 and not self.plural and self.time == "present":
            res = "{} is {} {}".format(str(self.subject),str(self.adject),str(self.remain))
            self.result.append(res)
            print("11",res)
        return res
    ...
    def expressTime(self):
        ...
        if self.verb and self.verb in engVerbs:
            self.verbPast = [(x[0],x[1]) for x in englishPastParticiples if x[0] == self.verb]
...
class Clause:
    vagueStop = ['about', 'kind of', 'something','somehow', 'and that kind of thing', 
'what do you call it', 'that stuff','the thing', 'whatyamacallhim',
'sort of','kind of', 'in or around', 'more or less', 'four-ish',
'and things like that', 'and that kind of thing','and stuff like that',
'and that sort of thing and stuff', 'and that type of thing and so on',
'and things like that and this', 'that and the other','and the like'
'whatever', 'whoever', 'whenever', 'whichever']
    
    modalStop = ['can','cannot','could',"couldn't",'may','might',
                 'must','shall','should',"shouldn't",'would',"wouldn't",'ought']
 
    engSUBPR = ['i','you','he','she','it','we','they','that','this','these','those']
    engObjPronouns = ['me','you','him','her','it','us','you','them']
    engPOSPR = {'i':'my','you':'your','he':'his','she':'her','it':'its','we':'our','they':'their',
                'this':'its','that':'its','these':'their','those':'their'}
    engOBJPR = {'i':'me','you':'you','he':'him','she':'her','it':'it','we':'us',
                'they':'them','this':'it','that':'it','these':'them','those':'them'}
    engPosObjConvert = {'me':'my','you':'your','him':'his','her':'her','it':'its','us':'our','them':'their'}
    case = 0
    results = []
    ...
    def __init__(self,part,cc):
        ...
        if "VERB-CCONJ-VERB" in self.allposesPattern and (self.allwords[self.allposes.index('CCONJ')] == 'and' or self.allwords[self.allposes.index('CCONJ')] == 'or'):
        ...
        elif "CCONJ-VERB" in self.allposesPattern and self.allwords[self.allposes.index('CCONJ')] == 'and' and self.alltags[self.allposes.index('CCONJ')+1] != 'VBG':
            ...
            if cutFirstRes and cutSecondRes and 'Error' not in cutFirstRes or 'Skipped' not in cutFirstRes and 'Error' not in cutSecondRes or 'Skipped' not in cutSecondRes:
                mixThem = "{} and {} ".format(cutFirstRes, cutSecondRes)
                self.result.append([mixThem])
            else:
                pass
        
        elif "CCONJ-NOUN-VERB" in self.allposesPattern or "CCONJ-NOUN-ADV-VERB" in self.allposesPattern or "CCONJ-PRON-VERB" in self.allposesPattern or "CCONJ-PRON-ADV-VERB" in self.allposesPattern:
            ...
            while i < len(cc)-1:
                ...
                if cutRoot:
                    ...
                    if cutRootCheck.startswith('NP-VP') or cutRootCheck.startswith('NPP-VP') or cutRootCheck.startswith('NPTH-VP') or cutRootCheck.startswith('NP-NPTH-VP'):
                        cutSecondRes = smallClause(" ".join(cutSecondPart))
        ...
        if "MD" in self.alltags:# and self.alldeps[self.alltags.index("MD")+1] == "ROOT":
            self.modal = True
        ...
        if 'NNP' in self.nounMTXbeforeRoot or 'NNPS' in self.nounMTXbeforeRoot:
            ...
            if self.taggedSent[1][0].startswith('The'):
                ...
                self.subject = self.taggedSent[1][0][0].lower()+self.taggedSent[1][0][1:]
                #print(self.subject)
            else:
        ...
        if 'OBD' in self.patternList and self.dict['OBD'].lower() in Clause.engObjPronouns:
            self.humanOBD = True
        else:
            self.humanOBD = False

        """Pronoun Indirect Object"""
        if 'OBI' in self.patternList and self.dict['OBI'].lower() in Clause.engObjPronouns:
            self.humanOBI = True
        else:
            self.humanOBI = False
        ...
        self.neg = False
        if 'not' in self.dict['VP'] or "n't" in self.dict['VP'] or ("neg" in self.alldeps and str(self.allheads[self.alldeps.index("neg")]) == str(self.myRoot)) or ("neg" in self.alldeps and "acomp" in self.alldeps and str(self.allheads[self.alldeps.index("neg")]) == str(self.words[self.alldeps.index("acomp")])):
    ...
    def verbProcess(self):
        ...
        if self.neg and not self.secondVerb:
            ...
            if self.root == "have":
                ...
                if self.verbAnt:
                    self.verbnomal = Clause.nomalize(Clause.antony(self,str(self.root),'v'),None)
                    if self.verbnomal:
        ...
        if Clause.case in [2,3,4,5]:
            ...
            if self.passive and self.neg:
                if self.humanSubPR:
                    ...
                    res = "the",self.subject,"not to be",self.myVerb," ".join(remain)
                    res = " ".join(res)
    ...
    def phraseGenerator(self):
        ...
        elif (self.pattern == 'NP-VP' or self.pattern == 'NPP-VP') and self.root == 'be' and not self.neg:
            Clause.case == 4
        ...
        if Clause.case == 1:
            ...
            if self.secondAdjective:
                ...
                remain = self.allwords[self.allwords.index(self.dict.get('ADJP')[-1])+1:]
            ...
            if self.human and len(self.subject.split()) == 1 and not self.neg:
                res = [str(self.subject),"'s",str(self.advpRBR),str(self.adjectnomal[0])," ".join(remain),str(self.advp)]
                self.result.append(res)
                ...
                res = ["the",str(self.advpRBR),str(self.adjectnomal[0]),"of the",str(self.subject)," ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1 and self.negING:
                res = ["the",str(self.advpRBR),str(self.subject),str(self.adjectnomal[0])," ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.pospron and not self.neg:
                res = [self.pospron,str(self.advpRBR),str(self.adjectnomal[0])," ".join(remain),str(self.advp)]
                self.result.append(res)
                ...
                if self.negING and not self.neg:
                    res = [str(self.advpRBR),str(self.subject),str(self.adjectnomal[0])," ".join(remain),str(self.advp)]
                    self.result.append(res)
                elif not self.negING and self.neg:
                    res = [str(self.advpRBR),str(self.adjectnomal[0]),"of the",str(self.subject),remain,str(self.advp)]
                    self.result.append(res)
                else:
                    res = ["the", str(self.advpRBR), str(self.adjectnomal[0]),"of the",str(self.subject), " ".join(remain),str(self.advp)]
                    self.result.append(res)
            ...
            if self.human and len(self.subject.split()) == 1:
                res = [self.subject,"'s", str(self.advpRBR),self.verbnomal[0],ofword,remain,str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1 and self.neg:
                res = [str(self.advpRBR),self.subject,self.verbnomal[0],remain,str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1:
                res = ["the",str(self.advpRBR),self.verbnomal[0],ofword,remain,"by",self.subject,str(self.advp)]
                self.result.append(res)
            elif self.pospron:
                res = [self.pospron, str(self.advpRBR),self.verbnomal[0],ofword,remain,str(self.advp)]
                self.result.append(res)
            elif self.passive:
                res = [self.pospron, str(self.advpRBR),self.verbnomal[0],ofword,remain,str(self.advp)]
                self.result.append(res)
            ...
            #Generate Sentence
            if self.human and len(self.subject.split()) == 1:
                res = [self.subject,"'s", str(self.advpRBR),self.verbnomal[0]," ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1 and self.neg:
                res = [str(self.advpRBR),self.subject,self.verbnomal[0],ofword, " ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1:
                res = ["the",str(self.advpRBR),self.verbnomal[0],"of the", self.subject, " ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.pospron and self.neg and self.verbnomal[0] == 'lack':
                res = [self.pospron,self.verbnomal[0],"of",str(self.advpRBR)," ".join(remain), str(self.advp)]
                self.result.append(res)
            elif self.pospron:
                res = [self.pospron, str(self.advpRBR),self.verbnomal[0], " ".join(remain), str(self.advp)]
                self.result.append(res)
            else:
                if self.neg:
                    res = [str(self.advpRBR),self.subject,self.verbnomal[0]," ".join(remain),str(self.advp)]
                    self.result.append(res)
                else:
                    res = ["the",str(self.advpRBR), self.verbnomal[0], "of the", self.subject," ".join(remain),str(self.advp)]
                    self.result.append(res)
            ...
            #Generate Sentence
            if self.human and len(self.subject.split()) == 1:
                res = [self.subject,self.verbnomal[0],str(self.advpRBR)," ".join(remain),str(self.advp)]
                self.result.append(res)
            elif self.human and len(self.subject.split()) > 1:
                res = [self.subject,self.verbnomal[0],str(self.advpRBR)," ".join(remain),"by",str(self.advp)]
                self.result.append(res)
            elif self.pospron:
                res = [self.pospron, str(self.advpRBR),self.verbnomal[0]," ".join(remain),str(self.advp)]
                self.result.append(res)
    ...
    def processRules(self):
       ...
       assigned in this function. """
        ...
        #Set the cases
        case1 = ['NP-VP-ADJNM','NP-VP-ADJP-ADJNM','NP-VP-ADJP-ADVP','NP-VP-ADJP','NP-VP-ADJP-OBI',
                 'NP-VP-ADJP-OBI-ADVP','NP-VP-OBPRD','NP-VP-ADJP-OBI-ADJNM-ADVP','NP-VP-ADJP-OBD','NP-VP-ADJP-OBD-OBI','NP-VP-ADJP-OBD']
        case2 = ['NP-VP-OBD','NP-VP-OBD-ADVP','NP-VP-OBD-OBI','NP-VP-OBD-OBAPPOS','NP-VP-OBD-OBI-OBAPPOS','NP-VP-OBD-OBI-OBAPPOS-ADVP','NP-VP-OBD-OBAPPOS-ADVP','NP-VP-OBPRD-OBI']
        case3 = ['NPP-VP-OBI','NP-VP','NPP-VP','NPP-VP-ADVP','NP-VP-OBI','NPP-VP-OBI-OBAPPOS',
                 'NPP-VP-OBPRD','NP-VP-OBI-ADVP','NP-VP-OBI-NPAtt','NP-VP-ADVP','NPP-VP-OBI-ADVP','NPP-VP-OBD']
        case4 = ['NP-VP-NPAtt','NPTH-VP-OBI','NP-NPTH-VP-OBI'] #e.g. She is a poor talker
        case5 = ['NPTH-VP-NPAtt','NP-NPTH-VP-NPAtt','NP-NPTH-VP-OBI-NPAtt','NP-NPTH-VP-OBD-NPAtt']
        ...
        if self.pattern in case1:
            ...
            if "ADJ-CCONJ-ADJ" in self.allposesPattern and self.allwords[self.allposes.index('CCONJ')] == 'and':
                ...
                if self.neg:
                    self.adjectnomal = Clause.adjectify(Clause.antony(self,self.adjectmain,'a'))
                    print('962',self.adjectmain, self.adjectnomal[0])
                else:
                    self.adjectnomal = Clause.adjectify(self.adjectmain)
                #print('961',self.adjectnomal)
            
            elif "ADJ-CCONJ-ADV-ADJ" in self.allposesPattern and self.allwords[self.allposes.index('CCONJ')] == 'and':
                myIndex = self.allposes.index('CCONJ')
                firstAdj = self.allwords[myIndex-1]
            ...
            elif self.dict.get('ADJP') and len(self.dict.get('ADJP')) >= 1 and self.neg == False:
            ...
            elif self.dict.get('ADJNM') and len(self.dict.get('ADJNM')) >= 1 and self.neg == False:
            ...
            elif self.dict.get('ADJP') and len(self.dict.get('ADJP')) >= 1 and self.neg == True:
                self.adjectmain = self.dict.get('ADJP')[-1]
            ...
            elif self.dict.get('ADJNM') and len(self.dict.get('ADJNM')) >= 1 and self.neg == True:
                self.adjectmain = self.dict.get('ADJNM')[-1]
                a = Clause.adjectify(Clause.antony(self,self.adjectmain,'a'))
                if a:
                    b = ' '.join(a)
            ...
            if (self.pattern == 'NP-VP' or self.pattern == 'NP-VP-OBI' or self.pattern == 'NP-VP-OBD') and (self.root == 'be' or self.root=="'s" or self.root=="'re") and self.subject in ['it','this','that','those','these','there']:
                Clause.case = 0
                #self.erCode = 104
            else:
                Clause.case = 3
    ...
    def adverbWork(self):
        ...
        for possible_adv in matrix:
            if possible_adv.dep == advmod and possible_adv.head.dep_ != "relcl" and possible_adv.pos_ != "ADJ" and not possible_adv.text.startswith('wh'):
                self.adverbs.append([possible_adv.text, str(possible_adv.head), possible_adv.head.dep_, words.index(possible_adv.text)])
        ...
        if self.adverbs: 
            ...
            for advb in enumerate(self.adverbs):
                ...
                #join adverb head in e.g 'more easily'. Combine two adverbs take Deps from second. Adv2Adj converts both words
                    ...
                    self.adverbs = [x for x in self.adverbs if str(x[0]) != str(advb[1][1])] 
                else:
                    pass
            ...
            if Clause.case == 1:
                for adv in self.adverbs:
                    ...
                    if adv_text == "more" and (adv_headdep == "acomp" or adv_headdep == "ccomp" or adv_headdep == "xcomp"):
                        adv_convert = "greater"
    ...
    def getResult(self):
        ...
        if Clause.case != 0:
            ...
            if self.result:
                ...
                res = " ".join(res)
                return res, Clause.case, self.erCode,self.posCovertion,self.PrepPhrase,self.advpInit,self.relCheck,self.participle,self.query
            else:
                sent = '{}'.format(self.sentence)
                return sent, Clause.case, self.erCode,self.posCovertion,self.PrepPhrase,self.advpInit,self.relCheck,self.participle,self.query
        else:
            sent = '{}'.format(self.sentence)
            self.posCovertion = []
            self.participle = []
...
def expressionTime(sent):
    ...
    tense_list = {'VB':'base','VBD':'past','VBG':'gerund','VBN':'pastp', 'VBP':'present', 'VBZ':'third'}
    ...
    elif (rootTense == 'gerund' or rootTense == 'pastp' or rootTense == 'base') and mtx[3].tolist()[rootIndex-1]:
        prev_dep = mtx[1].tolist()[rootIndex-1]
        if prev_dep == "neg": #if previous tag is neg go one word back further to the modal verb
    ...
    # If there is no mark, cuts the sentence based on Nsubj/Nsubjpass location
        ...
        if len(cutPoints) == 1:
            ...
            for r in result:
                ...
                n = 0 
                if len([i for i, x in enumerate(cdeps) if x == "nsubj" or x == "nsubjpass"]) == 1:
                    if "punct" in cdeps:
...
it returns False, if the sentence is not causative."""
    ...
    keywords = ['after','After','before','Before','until','Until','once','Once','even though','Even though','though','Though','due to','Due to','owing to the fact that','Owing to the fact that','as a result of','As a result of','as a result','As a result','because of','Because of', 'because','Because', 'thus','Thus', 'therefore','Therefore', 'since','Since', 'consequently','Consequently', 'but','But', 'although', 'Although','while','While','despite','Despite','even if','Even if','if','If','even when','Even when','even yet','Even yet','whether','Whether','whereas','Whereas','and so','so']
    
    keywordLocation = [x for x in words if x in keywords and (deps[words.index(x)] == 'mark' or deps[words.index(x)] == 'advmod' or deps[words.index(x)] == 'intj' or deps[words.index(x)] == 'cc')]
    if keywordLocation:
        pass
    else:
        return False
    ...
    if 'mark' in deps and words[deps.index("mark")] in keywords:
        ...
        compound = '{} {}'.format(preWord,markWord) #needs this to get full keyword to calculate length of keyword
    ...
    if result:
        ...
        if status == "start" and len(subjsIndice)> 1:
            lefts = [(subjsIndice[1]-1,deps[subjsIndice[1]-1]),(subjsIndice[1]-2,deps[subjsIndice[1]-2]),(subjsIndice[1]-3,deps[subjsIndice[1]-3])]
            for x in lefts:
                if 'punct' in x:
                    secondSubjCut = x[0]   
    ...
    midKeys = ['therefore','affect','cause','happen','consequently','however','still','whereas','thus','and so','so']
    startmidKeys = ['when','When','after','After','before','Before','until','Until','once','Once','as soon as','As soon as','as','As','even though','Even though','though','Though','due to','Due to','owing to the fact that','Owing to the fact that', 'since', 'Since','even if','Even if', 'if','If', 'as a result of','As a result of', 'because of','Because of', 'as a result','As a result', 'even yet','Even yet', 'consequently', 'Consequently','while','While', 'but','But','because','Because', 'although','Although','whether','Whether', 'whereas','Whereas', 'even when','Even when', 'despite','Despite']
...
def NpExtractNumpy(sent,req):
    ...
    if rootNextDep == 'prep':
        limiters = ['neg','predet', 'pcomp', 'prt', 'dobj', 'advmod', 'npadvmod', 'det', 'amod', 'cc', 'conj', 'compound', 'poss','nummod','attr', 'punct','poss','case','pobj','auxpass','relcl'] 
    else:
        limiters = ['neg','predet', 'pcomp','prt','dobj', 'npadvmod', 'advmod','det', 'amod', 'cc', 'conj', 'compound', 'poss', 'nummod', 'pobj', 'attr', 'prep','punct','poss','case','auxpass','relcl']      
    ...
    #check indirect object be after verb not before it because nsubj/nsubjpassive phrase may include pobj
        ...
        limiters = ['advmod','npadvmod','amod', 'agent', 'punct', 'quantmod', 'compound', 'nummod', 'poss', 'case', 'attr', 'appos', 'det']
    ...
    for x in itemList:
        ...
        for i in range(x-1,-1,-1):
            ...
            #print(preword)
            #print(prepos)
            if predep in limiters and predep != 'pcomp' and predep != 'ROOT' and preword not in notallowedPreword:
                pre.append([i]) 
                #print(pre)
            else:
                break
...
def adverbExtract(sent,req):
    ...
    for i in itemList:
        ...
        if rbhead != root and rbworddep != "relcl" and rbheadtag =='RB' or rbheadtag =='RBR':
...
def sentTagger(sent):
    ...
    vbPhrase = findVerb[0]
    myPrep = findVerb[1]
    npIndicators = ['nsubj','nsubjpass','dobj','pobj','attr','oprd','appos','expl','nmod','xcomp','csubj']
    result = []
    parts = []
    npSubject = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'nsubj' or x == 'csubj']
    npPsubject = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'nsubjpass']
    npDobject = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'dobj' or x == 'xcomp']
    npPobjects = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'pobj']
    npOpr = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'oprd']
    npAttr = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'attr']
    npAppos = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'appos']
    npExpl = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'expl']
    npNmod = [NpExtractNumpy(sent,x) for x in arr[1] if x in npIndicators and x == 'nmod']
    ...
    if npPobjects and npPobjects[0] != 'None':
        for a in npPobjects[-1]:
            if a[1] > rootIndex:
                ...
                if (npSubject and a[0] not in npSubject[0]) or (npPsubject and a[0] not in npPsubject[0]):
                    npPrep.append(a[0])
...
def conditionalSent(sent):
    ...
    if mixPattern == "present-present" and not firstAuxFind and not secondAuxFind:
        ...
        sent = "<span class='causePart'> {} </span> will result in <span class='effectPart'> {} </span>".format(cause,secondNom[0])
    elif mixPattern == "past-present" and secondAuxFind and ("has" in secondPart.split() or "have" in secondPart.split() or"had" in secondPart.split()):
        condType = '3'
        cause = firstNom[0]
        cause = cause[0].upper()+cause[1:]
        sent = "<span class='causePart'> {} {} </span> result in <span class='effectPart'> {} </span>".format(cause,secondAuxFind[0],secondNom[0])
        ...
        cause = firstNom[0]
        cause = cause[0].upper()+cause[1:]
        sent = "<span class='causePart'> {} </span> would result in <span class='effectPart'> {} </span>".format(cause,secondNom[0])
...
def causativeSent(sent,cause,effect,c,e,p,r):
    ...
    #c = c[0].upper()+c[1:]
    c1 = c[0].upper()+c[1:]
    res = "<span class='causePart'> <span class='tooltiptext'>Cause Clause</span> {} {} </span> {} <span class='effectPart'> <span class='tooltiptext'>Effect Clause</span> {} </span>.".format(p,c1,relword1,e)
    res = res.replace(" None ", "")

    """ This part handles the second type of causative output sentence"""
    effect = effect[0].upper()+effect[1:]
    #c = c[0].lower()+c[1:]
    res2 = "<span class='effectPart'> <span class='tooltiptext'>Effect Clause</span> {} {} </span> {} <span class='causePart'> <span class='tooltiptext'>Cause Clause</span> {} </span>.".format(p,effect,relKeyword,c) 
    ...
    e = e[0].upper()+e[1:]
    #c = c[0].lower()+c[1:]
    res3 = "<span class='effectPart'> <span class='tooltiptext'>Effect Clause</span> {} {} </span> {} <span class='causePart'> <span class='tooltiptext'>Cause Clause</span> {} </span>.".format(p,e,relword2,c)
...
def temporalSent(sent,cause,effect,c,e,p,key,location):
    ...
    effect = effect[0].upper()+effect[1:]
    res = "<span class='effectPart'><span class='tooltiptext'>Main Clause</span> {} </span> <b> {} </b> <span class='causePart'><span class='tooltiptext'>Subordinate Clause</span> {} </span>".format(effect,key,c)
    res = res.replace(" None ", "")
...
def concessSent(sent,cause,effect,c,e,p):
    ...
    e1 = e[0].upper()+e[1:]
    #c = c[0].lower()+c[1:]
    res = "<span class='effectPart'><span class='tooltiptext'>Effect Clause</span> {} {} </span> {} <span class='causePart'><span class='tooltiptext'>Calim Clause</span>  {} </span>.".format(p,e1,relword,c)
    res = res.replace(" None ", "")
    
    effect = effect[0].upper()+effect[1:]
    #c = c[0].lower()+c[1:]
    """ This part handles the second type of concessive output sentence"""
    res2 = "<span class='effectPart'><span class='tooltiptext'>Effect Clause</span> {} {} </span> despite <span class='causePart'><span class='tooltiptext'>Claim Clause</span>  {} </span>.".format(p,effect,c) 
    res2 = res2.replace(" None ", "")
...
def cline(sentence):
    ...
    #try:
        ...
        errors = {101:'No subject',102:'No passive subject',103:'Incorrect Parsing',104:'NP+BE Clause',105:'Unknown Structure',107:'Modal verb'}
        if cause == True:
            c = None
            e = None
            #c_pos = ''
            ...
            moveRes = moveTable((clauses[0][0],c),(clauses[1][0],e),key,participlePhrase,sen_time) #### MOVEMENT TABLE STARTS HERE
        else:
            ############################################################################################################
            ...
            if cCheck:
                ...
                moveRes = []
                return res,res2,posConversion,clauseCases,clauseErrors,webColor,prChange,moveRes,notFormattedResult
            
        if cause == True and not any(x in [101,102,103,104,105,107] for x in clauseErrors):
            ...
            if concess == False:
                ...
                cause_res = causativeSent(sent,clauses[0][0],clauses[1][0],causePart[0],effectPart[0],prepPhrase[0],relCheck)
                #print("RES",cause_res)
                res = cause_res[0]
                res2 = cause_res[1]
                res3 = cause_res[2]
            ...
            #Handles Concessive Result Generation
                ...
                concess_res = concessSent(sent,clauses[0][0],clauses[1][0],causePart[0],effectPart[0],prepPhrase[0])
                res = concess_res[0]
                res2 = concess_res[1]
                res3 = concess_res[2]
                res4 = concess_res[3]
            ...
            if temporal == True:
                temp_res = temporalSent(sent,clauses[0][0],clauses[1][0],causePart[0],effectPart[0],prepPhrase[0],key, templocation)
                res = temp_res[0]
                res2 = temp_res[1]
                res3 = temp_res[2]
                res4 = temp_res[3]
            ...
            if initAdverb:
                ...
                #myadverb = matrix1[0][myadverbIndex].lower()
                    ...
                    webColor = webColoring(sentence,res,res2,res3,causePOSConversion,key)
...
def clineNoEffect(sentence):
    ...
    try:
        ...
        errors = {101:'No subject',102:'No passive subject',103:'Incorrect Parsing',104:'NP+BE Clause',105:'Unknown Structure',107:'Modal verb'}
        if cause == True:
            if len(clauses[0][0].split()) > 1 and 'VERB' in [w.pos_ for w in nlp(clauses[0][0])]:
                c = Clause(clauses[0][0],'C').getResult()
                #print(c)
                causePart.append(c[0])
                clauseCases.append(c[1])
            ...
            prChange = []
            clauseErrors = [106,'Not Causative']
            return sentence,res2,posConversion,clauseCases,clauseErrors,webColor,prChange
        ...
        if cause == True and not any(x in [101,102,103,104,105,107] for x in clauseErrors):
            ...
            if concess == False:
                """ This part handles the first type of causative output sentence"""
                res = '{} {} {} {}.'.format(prepPhrase[0],causePart[0],sent_time,effectPart[0])
                res = sentCleaner(res)
                ...
                """ This part handles the second type of causative output sentence"""
                res2 = '{} {} {} {}.'.format(prepPhrase[0],clauses[1][0],relKeyword,causePart[0]) 
                res2 = sentCleaner(res2)
            ...
            #Handles Concessive Result Generation
                ...
                res = '{} {} {} in spite of {}.'.format(prepPhrase[0],effectPart[0],myverb,causePart[0])
                res = sentCleaner(res)

                """ This part handles the second type of concessive output sentence"""
                res2 = '{} {} despite {}.'.format(prepPhrase[0],clauses[1][0],causePart[0]) 
                res2 = sentCleaner(res2)
            ...
            if initAdverb:
                ...
                if myadverb:
                    ...
                    prChange = []#replacePronoun(res2)
                    return res,res2,posConversion,clauseCases,clauseErrors,webColor,prChange
                else:
                    res = res[0].upper()+res[1:]
                    res2 = res2[0].upper()+res2[1:]
                    """ Extra Web Work for Res2 output """
